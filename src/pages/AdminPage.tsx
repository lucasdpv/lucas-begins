...
export default function AdminPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (type === 'message') {
        ...
        const isUsed = (posts as Post[]).some((p) => p.category === id);
        if (isUsed) {
  ...
  const handleAddCat = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCategoryName.trim() || (categories as string[]).includes(newCategoryName.trim())) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12">
          {[
            ...
            { label: "Categorias", value: (categories as string[]).length, icon: Tag, color: "text-purple-500", bg: "bg-purple-500/10" },
          ...
          <div className="flex flex-col md:flex-row gap-3 md:gap-4 items-center justify-between mb-4 md:mb-6">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              <div className={cn(
                ...
                (posts as Post[]).filter(p => p.isFeatured).length >= 5 
                  ? "bg-yellow-500/10 border-yellow-500 text-yellow-500" 
                  : "bg-purple-500/10 border-purple-500 text-purple-500"
              )}>
                Carrossel: {(posts as Post[]).filter(p => p.isFeatured).length} / 5
              </div>
            </div>
          </div>
                      ...
                      >
                        ...
                        <td className="px-4 md:px-6 py-3 md:py-4 text-right">
                          <div className="flex items-center justify-end gap-2 md:gap-3">
                            <button
                                onClick={() => {
                                  if (!post.isFeatured && (posts as Post[]).filter(p => p.isFeatured).length >= 5) {
